"""Deterministic resolver: verbatim spans -> canonical, usable event payload.

The model only copies spans. This module turns those spans into the flat payload
the Calendar layer consumes: ISO start, duration in minutes, reminder offsets in
minutes, plus a `complete` flag and the fields it could not resolve. A closed
grammar covers Romanian, Russian and English; anything outside the grammar stays
null and is reported in `unresolved`, never guessed.

Reference date is fixed for the benchmark so relative spans ("tomorrow",
"Marti") resolve to the same instant every run.
"""
...
REFERENCE_DATE = date(2026, 9, 18)  # Friday, the benchmark run day
...
def parse_date(text: str):
    ...
    for token in tokens:
        if token in REL:
            return REFERENCE_DATE + timedelta(days=REL[token])
    for token in tokens:
        if token in WEEKDAYS:
            delta = (WEEKDAYS[token] - REFERENCE_DATE.weekday()) % 7 or 7
    ...
    for token in tokens:
        if token in MONTHS:
            ...
            day = ORD[token]
    if day and month:
        result = date(REFERENCE_DATE.year, month, day)
        if result < REFERENCE_DATE:
            result = date(REFERENCE_DATE.year + 1, month, day)
        return result
    return None
...
def resolve(extraction: dict, reference: date = REFERENCE_DATE) -> dict:
...
def _norm(value):
    ...
    if isinstance(value, str):
        return score.normalize(value)
    return value


def compare(expected: dict, actual: dict):
    reasons = []
    for field in ("operation", "event_type", "title", "start", "all_day",
                  "duration_minutes", "location"):
        if _norm(expected[field]) != _norm(actual[field]):
            reasons.append(field)
    if expected["recurrence"] != actual["recurrence"]:
        reasons.append("recurrence")
    if expected["reminders_minutes"] != actual["reminders_minutes"]:
        reasons.append("reminders")
    usable = bool(actual["complete"])
    return usable, usable and not reasons, reasons
